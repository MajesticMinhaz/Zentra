"""
PDF generation for quotes using WeasyPrint.
Company identity is pulled from quote.organization (falls back to settings).
"""
import os
import logging
...
PDF_OUTPUT_DIR = Path(settings.MEDIA_ROOT) / "quotes" / "pdf"


def _get_org_context(quote):
...
def render_quote_pdf(quote) -> str:
    """
    Renders a quote to PDF using WeasyPrint and saves to disk.
    Returns relative path to the PDF file.
    """
    try:
    ...
    context = {
        "quote": quote,
        "line_items": quote.line_items.select_related("item").all(),
        "base_url": str(Path(settings.MEDIA_ROOT)),
        **_get_org_context(quote),
    }

    html_string = render_to_string("quotes/pdf_quote.html", context)
    ...
    logger.info("Quote PDF saved: %s", output_path)
    return relative